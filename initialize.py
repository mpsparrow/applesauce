...
async def on_ready():
    ...
    logger.outputWrite('Initializing Required Extensions') # output-log.txt
    for required in os.listdir('./cogs/required'): # looks in /cogs/required
        if required.endswith('.py'): # if a .py file is found
            try:
                ...
                logger.outputWrite('killing started') # output-log.txt
                return
    ...
    for addons in os.listdir('./cogs/addons'): # looks in /cogs/addons
        if addons.endswith('.py'): # if a .py file is found
            if addons[:-3] not in configloader.config['main']['excludedModules']: # if module doesn't exist in excludedModules list (config.json)
                try:
                    bot.load_extension(f'cogs.addons.{addons[:-3]}') # load addon
                    logger.outputWrite(f' Successfully loaded {addons}') # output-log.txt
                    countSuccess += 1 # adds to successfully loaded addons
                except Exception as e:
                    logger.outputWrite(f' Failed to load {addons}') # output-log.txt
                    logger.outputWrite(f'  Error: {e}') # output-log.txt
                    countFail += 1 # adds to failed to load addons

    logger.outputWrite(f' Addons Loaded: Success: {countSuccess} Failed: {countFail}\n') # output-log.txt
