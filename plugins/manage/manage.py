    ...
    async def enable(self, ctx, plug, guildID=None):
        ...
        try:
            ...
            if guildID is not None and await self.bot.is_owner(ctx.author):
                ...
                if validID:
                    pluginCol.update_one({ "_id": plug }, { "guild": { "$set": { str(guildID): True }}}, upsert=True)
                ...
                pluginCol.update_one({ "_id": plug }, { "guild": { "$set": { str(ctx.guild.id): True }}}, upsert=True)
    ...
    async def disable(self, ctx, plug, guildID=None):
        ...
        try:
            ...
            if guildID is not None and await self.bot.is_owner(ctx.author):
                ...
                if validID:
                    pluginCol.update_one({ "_id": plug }, { "guild": { "$set": { str(guildID): False }}}, upsert=True)
                ...
                pluginCol.update_one({ "_id": plug }, { "guild": { "$set": { str(ctx.guild.id): False }}}, upsert=True)
